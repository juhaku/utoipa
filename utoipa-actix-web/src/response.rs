...
//! use utoipa::OpenApi;
//! use utoipa_actix_web::response::{Created, Status};
//!
...
//! }
//!
//! #[utoipa::path(responses(Created<Json<Pet>>))]
//! #[post("/pet")]
...
//! use utoipa_actix_web::response::{status, Status};
//!
...
pub mod status {
    macro_rules! status_codes {
        ( $( $(#[$doc:meta])* $name:ident = $code:literal; )* ) => {
            ...
            #[cfg(test)]
            #[test]
            fn status_codes_match_status_code_constants() {
                $(
                    assert_eq!(
                        $name,
                        actix_web::http::StatusCode::$name.as_u16(),
                        stringify!($name)
                    );
                )*
            }
    ...
    // `StatusCode::as_u16` of the `http` version used by actix-web is not a `const fn`, so the
    // codes are literals checked against `StatusCode` by a test.
    status_codes! {
        /// `100 Continue`
        CONTINUE = 100;
...
///
/// The status code of the response of `B` is replaced with `CODE`, the same way as
/// [`CustomizeResponder::with_status`][actix_web::CustomizeResponder::with_status] does.
...
impl<const CODE: u16, B: Responder> Responder for Status<CODE, B> {
    type Body = B::Body;
...
mod tests {
    ...
    fn created_json_registers_referenced_schemas_in_components() {
        ...
        #[utoipa::path(responses(Created<Json<Item>>))]
        #[post("/item")]
        #[allow(unused)]
    ...
    fn status_responds_with_status_code() {
        ...
        let response = created.respond_to(&request);
        assert_eq!(response.status(), StatusCode::CREATED);

        let response = NoContent.respond_to(&request);

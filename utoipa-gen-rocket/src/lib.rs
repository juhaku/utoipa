...
use utoipa_corelib::fn_arg::{FnArg, GenericType, TypeTree, ValueType};
use utoipa_corelib::{
    fn_arg, ArgValue, ArgumentIn, MacroArg, PathOperations, ResolvedOperation, ValueArgument,
};
...
pub trait PathOperationsExt {
    ...
    fn resolve_operation(item_fn: &ItemFn) -> Option<utoipa_corelib::ResolvedOperation>;
    fn resolve_path(path: &Option<String>) -> Option<utoipa_corelib::MacroPath>;
}
...
impl PathOperationsExt for PathOperations {
    ...
    fn resolve_operation(ast_fn: &syn::ItemFn) -> Option<utoipa_corelib::ResolvedOperation> {
            ...
            if is_valid_route_type(attribute.path.get_ident()) {
                ...
                if let Some(operation) = operation {
                    Some(ResolvedOperation {
                        path_operation: utoipa_corelib::PathOperation::from_str(&operation)
                            .unwrap(),
                        path,
                    })
                } else {
                    Some(ResolvedOperation {
                        path_operation: utoipa_corelib::PathOperation::from_ident(
...
fn to_anonymous_value_arg<'a>(macro_arg: MacroArg) -> utoipa_corelib::ValueArgument<'a> {
    let (name, argument_in) = match macro_arg {
        MacroArg::Path(arg_value) => (arg_value.name, utoipa_corelib::ArgumentIn::Path),
        MacroArg::Query(arg_value) => (arg_value.name, utoipa_corelib::ArgumentIn::Query),
    };
...
fn with_argument_in(
    named_args: &[MacroArg],
) -> impl Fn(FnArg) -> Option<(FnArg, utoipa_corelib::ArgumentIn)> + '_ {
    move |arg: FnArg| {
...
fn is_anonymous_arg(arg: &MacroArg) -> bool {
    matches!(arg,utoipa_corelib::MacroArg::Path(path) if path.original_name == ANONYMOUS_ARG)
        || matches!(arg,utoipa_corelib::MacroArg::Query(query) if query.original_name == ANONYMOUS_ARG)
}
